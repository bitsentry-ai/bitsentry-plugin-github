...
function resolveOwner(input, auth) {
  const owner = readString(
    input.owner,
    readString(input.orgSlug, readString(auth.orgSlug)),
  );
  if (owner.length === 0) {
...
function buildSearchQuery(input, owner, repo) {
  ...
  const normalizedQuery =
    query.length === 0 || query === "*" ? "is:issue" : query;
      ...
      setupFields: [
        {
          ...
          label: "GitHub token",
          description:
            "Personal access token or fine-grained token for GitHub.",
          required: true,
          control: "password",
        },
        {
